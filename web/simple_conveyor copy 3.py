...
def mask_red_strip(image):
    ...
    # Copy the red pixels to the output image
...
def calculate_velocity(image_before, image_now, timestamp_before, timestamp_now):
    ...
    v_0 = average_x_displacement / time_difference
        ...
        if timestampNow != timestampBefore:
            ...
            if cameraBefore_base64 and cameraNow_base64:
                try:
                    ...
                    print(f"Calculated velocity: {velocity} metros per second")
                    system_model.variableStates["sensorSpeed"] = str(velocity)
...
def pid_controller():
    Kp = 1.0; Ti = 0.2; Tt=Ti; Ts=0.5; ao = Ts/Tt
    uiBefore = 0
    ui = 0
    bi = Kp * Ts/Ti
    ao = Ts/Tt
